...
for i in 64 65 128 256 512 1024 1500 2048 4096 9000; do
	make -C ../../riscv_code/ NAME=latency TARGET=latency_$i SRC=latency.c DEFINES="-DSIZE=$i -DCONGESTION=0"
...
# congested test
for i in 64 65 128 256 512 1024 1500 2048 4096 9000; do
	make -C ../../riscv_code/ NAME=latency TARGET=latency_$i SRC=latency.c DEFINES="-DSIZE=$i -DCONGESTION=1"
  sudo ./pcie_hot_reset.sh $pci_id
  sleep 1s
  sudo ip link set dev $eth up
  sudo ./rvfw -d /dev/$dev -i ../../riscv_code/latency_$i\_ins.bin -e 0xffff -r 0xaaaa -m ../../riscv_code/latency.map
