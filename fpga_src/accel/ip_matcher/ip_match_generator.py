...
prefix_table = sorted(prefix_table, key=lambda x: len(x), reverse=True)
...
for x in prefix_table:
	...
	if k in MSB_dict:
		if (len(x)!=first_lvl_len):
			MSB_dict[k].append(x[first_lvl_len:])
	else:
		if (len(x)==first_lvl_len):
			# since list is sorted based on number of wildcards
			first_lvl.append(x)
		else:
			MSB_dict[k] = [x[first_lvl_len:]]

with open(mod_name+'.v',"w") as f:
  ...
  for k in sorted(MSB_dict):
  		...
  		f.write('    casex (addr)\n')
  		for x in MSB_dict[k]:
  			f.write('      '+str(max_prefix_len-first_lvl_len)+'\'b'+x+fill[max_prefix_len-first_lvl_len-len(x)]+': '+'match <= 1\'b1;\n')
