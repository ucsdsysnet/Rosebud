...
out_verilog = "fixed_loc_rules"+str(line_size)+".v"
  ...
  for c in content[1:-1]:
    if (c=='|'):
      if (byte_mode==0):
        ...
        res.append(("(data[8*"+str(loc%line_size)+"+:8] == 8'h"+temp+")",loc))
        loc += 1
        temp = ""
        byte_mode = 0
    else:
      if (byte_mode==0):
        if (nocase==True):
          res.append(("((data[8*"+str(loc%line_size)+"+:8] == 8'd"+str(ord(c.lower()))+")||(data[8*"+str(loc%line_size)+"+:8] == 8'd"+str(ord(c.upper()))+"))",loc))
        else:
          res.append(("(data[8*"+str(loc%line_size)+"+:8] == 8'd"+str(ord(c))+")",loc))
        loc += 1
      else: 
        if (c==' '):
          res.append(("(data[8*"+str(loc%line_size)+"+:8] == 8'h"+temp+")",loc))
...
def byte_conv(rule, offset):
  ...
  if op in bit_ops:
    if op[0]=="!":
      res = res + "((data[8*"+str(loc%line_size)+"+:8*"+num+"] "+op[1]+" "+val+") == 0)"
    else:
      res = res + "((data[8*"+str(loc%line_size)+"+:8*"+num+"] "+op[0]+" "+val+") != 0)"
  elif op=="=":
    res = res + "(data[8*"+str(loc%line_size)+"+:8*"+num+"] == "+val+")"
  else:
    res = res + "(data[8*"+str(loc%line_size)+"+:8*"+num+"] "+op+" "+val+")"
...
ver_file.write ("module fixed_loc_rules (input clk, input rst, input ["+str(line_size)+"*8-1:0] data, input valid, output reg ["+str(rule_count)+"-1:0] match);\n\n")
for t in tmp_signals:
  ver_file.write ("reg "+t+";\n")
ver_file.write ("\n")

ver_file.write ("always @ (posedge clk) begin\n")
ver_file.write ("  if (valid) begin\n")
